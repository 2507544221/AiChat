...
class Node extends Model{
    ...
    public function getNodesList(){
        try{
            $res=$this->field('node_id as id,node_name as title,node_sort as sort,node_pid as pid,node_path,node_icon,add_time,is_menu')
                ->order('node_sort aes')->select()->toArray();
        }catch (\Exception $e){
        ...
        try {
            ...
            if (1 == $roleId) {
                $res = $this->field('node_id as id,node_name as title,node_pid as pid,node_path,node_icon')
                    ->where('is_menu', 2)->order('node_sort aes')->select()->toArray();
            } else {
                $roleModel = new Role();
                $roleInfo = $roleModel->getRoleInfoById($roleId)['data'];
                if (!empty($roleInfo)) {
                    $res = $this->field('node_id as id,node_name as title,node_pid as pid,node_path,node_icon')
                        ->whereIn('node_id', $roleInfo['role_node'])->where('is_menu', 2)->order('node_sort aes')->select()->toArray();
                }
            }
        }catch (\Exception $e) {
            Log::error($e->getMessage());
